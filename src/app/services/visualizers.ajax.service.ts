...
export class VisualizerAjax {
  ...
  getSalesByYear(): Observable<ChartDataModel> {
    return this.http.get<ChartEntity>(urlMapping.saleByYear).pipe(
      map(entity => {
  ...
  getAllRecords(): Observable<MatTableData> {
    return this.http.get<any>(urlMapping.allRecords).pipe(
      map(e => {
  ...
  getVisualForm(): Observable<VisualFormEntity> {
